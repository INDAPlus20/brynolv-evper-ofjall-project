...
use crate::{gui::display::{self, Align, Color, Point, Rect, Window}, harddisk::{self, fat32::FileInfo}, ps2_keyboard::{KeyCode, KeyEvent, Modifiers}, svec::SVec};
...
impl Widget for OpenDialog {
	...
	fn draw(&mut self, mut window: Window) {
        ...
        window.draw_string(Rect::new(title_bar_area.x, title_bar_area.y + 8, title_bar_area.width, 16), 1, false, Align::Center, "Open File", text_color, title_bar_color, None);
        ...
        for (i, entry) in self.current_entries.iter().enumerate() {
            ...
            window.draw_string(Rect::new(main_area.x + Self::MARGIN, y, main_area.width - Self::MARGIN * 2, 16), 1, false, Align::Left, entry.name.to_str(), fg, bg, None);
            y += 16;
        }
		...
		match event {
			Event::KeyEvent(event) => match event {
                KeyEvent {
                    ...
                    if self.current_entries.len() == 0 { return Response::Nothing }
                    ...
                    if unsafe { harddisk::fat32::get_file_info(&self.current_path).is_directory } {
                        self.current_entries = unsafe { harddisk::fat32::list_entries(&self.current_path).unwrap().into() };
				...
				KeyEvent {
					...
					if self.selected + 1 < self.current_entries.len() {
                        self.invalidate(Rect::new(Self::MARGIN * 2, Self::MARGIN + 32 + self.selected * 16, self.used_area().width - Self::MARGIN, 32));
						self.selected += 1;
					}
					Response::Nothing
				}
