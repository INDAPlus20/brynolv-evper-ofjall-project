...
use display::{Point, Window};

use super::Widget;
use crate::gui::display::{self, Color, Rect};
...
impl Widget for MessageBox {
	...
	fn draw(&mut self, mut window: Window) {
		...
		window.draw_rect(
			Rect::new(0, 0, self.size.x, title_bar_height),
			self.title_bar_color,
		);
		window.draw_rect(
			Rect::new(0, title_bar_height, self.size.x, text_area_height),
